...
import (
	...
	"battleservice/src/services/battle/roommgr/room/internal/scn/internal/birth"
	...
	"battleservice/src/services/battle/roommgr/room/internal/scn/internal/physic"
...
func (s *Scene) AddPlayer(player ISessionPlayer) {
	...
	seelog.Info("[登录] 添加玩家成功addplayer [", s.room.ID(), ",", scenePlayer.Name, "],", scenePlayer.ID, ",ballId:", msg.BallId, ",view:", scenePlayer.GetViewRect(), ",cell:", len(cells),
...
func (s *Scene) RemovePlayer(playerId types.PlayerID) bool {
	...
	seelog.Info("[注销] 删除玩家成功 [", s.room.ID(), "],", player.ID, " players:", len(s.Players), ";", oldstatus, ",exp:", player.GetExp())
	return true
}
