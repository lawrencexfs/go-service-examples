...
public class SnappyDecompressor
{
    ...
    public int[] ReadUncompressedLength(byte[] data, int offset , int count)
    {
        count = offset + count;
        int sum = 0, currentShift = 0;
        while ((data[offset] & bitMask) != 0 && offset < count)
        {
            sum = UpdateSum(data, offset, currentShift, sum);
            offset++;
            currentShift += 7;
    ...
    public bool Decompress(byte[] compressed, int compressedOffset, int compressedSize , byte[] uncompress , out int uncompressCount)
    {
        var sizeHeader = ReadUncompressedLength(compressed, compressedOffset , compressedSize);
        uncompressCount = sizeHeader[0];

        if(uncompressCount > uncompress.Length)
        {
            return false;
        }

        Decompress(compressed, sizeHeader[1], compressedSize + compressedOffset - sizeHeader[1], uncompress, 0, uncompressCount);
